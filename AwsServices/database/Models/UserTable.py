...
class UserTable:
    ...
    def update_password(self, user_data):
        try:
            response = self.table.update_item(
                ...
                ExpressionAttributeValues={":p": user_data["Password"]},
                ReturnValues="UPDATED_NEW",
            )
            if "Attributes" in response:
                return Response(body={'message': 'Password updated successfully!'}, status_code=200)
            else:
                return Response(body={'error': 'Invalid credentials!'}, status_code=401)
        except Exception as e:
            return Response(body={'error': str(e)}, status_code=500)
    ...
    def get_user(self, email):
        try:
            response = self.table.get_item(
                Key={'Email': email})
            response2 = self.table.scan()['Items']
    ...
    def get_all_user(self):
        try:
            ...
            return Response(body={'users': response}, status_code=200)
            # else:
            #     return Response(body={'error': 'User not found!'}, status_code=404)
        except Exception as e:
            print('Error:', e)
